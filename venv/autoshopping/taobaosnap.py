                ...
                for i in range(self.number):
                    try:
                        ...
                        if "'NoneType' object has no attribute 'group'" in crawler:
                            ...
                            print(
                                f'[{time.strftime("%H:%M:%S", time.localtime())} INFO]: 抢购失败, 错误信息如下: \n{err}\n将在{self.trybuy_interval}秒后重新尝试.')
                            is_success = False
                    if i == self.number - 1 and is_success == False:
                        ...
                        print(
                            f'[{time.strftime("%H:%M:%S", time.localtime())} INFO]: 抢购***{good_infos[good_id]["title"]}***成功, 已为您自动提交订单, 请尽快前往淘宝完成付款.')
                        # 电脑语音提示
                        for _ in range(5):
...
if __name__ == '__main__':
    ...
    """
    python taobaosnap.py --interval 0.1 --time 20:00:00:00000000 --l 5
